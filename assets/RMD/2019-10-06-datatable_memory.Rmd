...
```{r}
based     <- quote({dt_based$z = rnorm(1e6); dt_based})
mutated   <- quote(dt_mutate <- mutate(dt_mutate, z = rnorm(1e6)))
copied    <- quote(dt_copied <- copy(dt)[, z := rnorm(1e6)])
modified  <- quote(dt_modify[, z := rnorm(1e6)])
```
...
```{r}
data.table(` ` = c("dt", "Just Pointing to dt", "Copied Object"),
           Address = c(obj_addr(dt), 
                       obj_addr(just_pointing_to_dt),
                       obj_addr(copied_object)))
```

So this means, at this point, `dt` and `justing_pointing_to_dt` are both pointing to the same object. Normally, when you then go and do something to either one, like adding a variable, R then makes a copy. This means it will now have a different address since it is now a different (copied) object, as shown below.
...
bmodif <- bench::mark(eval(modified), iterations = 10)
```
...
```{r}
based_filter   <- quote({dt_based[dt_based$grp == 1, ]})
mutated_filter <- quote(dt_mutate <- filter(dt_mutate, grp == 1))
copied_filter  <- quote(dt_copied <- copy(dt)[grp == 1])
modify_filter  <- quote(dt_modify[grp == 1])
```
...
```{r}
based_mean   <- quote(tapply(dt_based$x, dt_based$grp, mean))
mutated_mean <- quote(summarize(group_by(dt_mutate, grp), mean(x)))
copied_mean  <- quote(dt_copied <- copy(dt)[, mean(x), by = "grp"])
modify_mean  <- quote(dt_modify[, mean(x), by = "grp"])
```
...
```{r}
based_loop   <- quote(lapply(dt_based[, c("x", "y")], function(x) tapply(x, dt_based$grp, mean)))
mutated_loop <- quote(summarize_all(group_by(dt_mutate, grp), mean))
modify_loop  <- quote(dt_modify[, lapply(.SD, base::mean), by = "grp"])
```
...
```{r}
profmem::profmem(dt_modify[, base::mean(x), by = "grp"]) %>% 
  data.frame %>% 
  select(bytes, calls)
```
